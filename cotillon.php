...
if ($_SERVER["REQUEST_METHOD"] == "POST") { 
    $id_cotillon_array = $_POST["id_cotillon"];
    $inventario_final_array = $_POST["i_f"];
    $venta_registrada_array = $_POST["venta_r"];

    for ($i = 0; $i < count($id_cotillon_array); $i++) {
        $id_cotillon = $id_cotillon_array[$i];
        ...
        $update_query = "UPDATE cotillon SET i_f = $inventario_final, fecha_actualizacion = NOW(), venta_r = '$venta_registrada'" . ($inventario_final === "NULL" ? "" : ", actualizado = 1") . " WHERE id_cotillon = $id_cotillon";
...
$consulta_inventario = "SELECT * FROM cotillon";
$resultado_inventario = $mysqli->query($consulta_inventario);

$consulta_fecha = "SELECT fecha_actualizacion FROM cotillon ORDER BY fecha_actualizacion DESC LIMIT 1";
...
$consulta_exportar = "SELECT id_cotillon, costo_tot, producto, mililitros, i_i, i_f, costo_act, venta_r, ((mililitros * i_i - mililitros * i_f) / 90) AS temp_venta_aproximada, (((mililitros * i_i - mililitros * i_f) / 90) - venta_r) AS temp_diferencia_venta, fecha_actualizacion FROM cotillon";
$resultado_exportar = $mysqli->query($consulta_exportar);

// Verifica si se solicitó la exportación a Excel
if (isset($_GET['exportar_excel'])) {
    // Crear archivo Excel
    $filename = 'Inventario_cotillon_' . date('Ymd') . '.xls';
    ...
    while ($fila_exportar = $resultado_exportar->fetch_assoc()) {
        echo "{$fila_exportar['id_cotillon']}\t{$fila_exportar['costo_tot']}\t{$fila_exportar['producto']}\t{$fila_exportar['mililitros']}\t{$fila_exportar['i_i']}\t{$fila_exportar['i_f']}\t{$fila_exportar['costo_act']}\t{$fila_exportar['venta_r']}\t{$fila_exportar['temp_venta_aproximada']}\t{$fila_exportar['temp_diferencia_venta']}\t{$fila_exportar['fecha_actualizacion']}\n";
    }
...
?>
  ...
  <link rel="icon" href="https://imgs.search.brave.com/pBUmqNmrfCEkLG_fIGVPj1HspRHgQ3BWR05x_C9Cl64/rs:fit:32:32:1/g:ce/aHR0cDovL2Zhdmlj/b25zLnNlYXJjaC5i/cmF2ZS5jb20vaWNv/bnMvZGM2MjYwZTUy/YTUxOGU1MWQ0Mzdh/OTU1N2FmMjBlOWRi/ZmUzZDBmYjhmNjhh/Mjg1ZDQ5OGVmMDUx/OGMxYTZjYi9sYWxv/bmphLmNvbS5teC8" type="image/png">
      ...
      <title>Consultar Cotillon</title>
        ...
        <form method="POST" action="">
            <table class="table table-striped" id="table" style="width: 100%;">
                ...
                <?php while ($info_inventario = $resultado_inventario->fetch_assoc()): ?>
                    <tr>
                        <td><?php echo $info_inventario['id_cotillon']; ?></td>
                    ...
                    <!-- Campos ocultos para identificar la fila -->
                            <input type="hidden" name="id_cotillon[]" value="<?php echo $info_inventario['id_cotillon']; ?>">
                            <!-- Campo editable i_f -->
...
<script>
  ...
  $(document).ready(function () {
    ...
    $('#exportarExcel').on('click', function () {
      ...
      window.location.href = 'excel_cotillon.php';
    });
  });
</script>
    ...
    $(document).ready(function () {
        $('#terminar').on('click', function () {
            if (confirm('¿Estás seguro de terminar el inventario?')) {
                ...
                $.ajax({
                    type: 'POST',
                    url: 'actualizar_inventario_c.php', // Reemplaza esto con la ruta correcta a tu script de actualización
                    success: function (response) {
